...
def checkout_pos(invoice: InvoiceCreate, customer_id: Optional[str] = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    frontend_paid = invoice.total_amount - invoice.outstanding_amount
        ...
        try:
            ...
            for variant_id, quantity, db_item in line_item_models:
                ...
                if actual_stock < 0:
                    ...
                    print(f"AUDIT: Stock went negative ({actual_stock}) - invoice {invoice_data.invoice_number}")
            ...
            synced_count += 1
            print(f"✅ Invoice {invoice_data.invoice_number} synced successfully")

        except Exception as e:
            db.rollback()  # ← rollback this invoice only, don't poison next one
            print(f"❌ Failed to sync invoice {invoice_data.invoice_number}: {e}")
            continue
